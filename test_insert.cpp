...
namespace diskann {
    ...
    extern void reset_latency_counters();
    ...
    extern std::vector<std::atomic<long long>> total_search_prune_ns_per_thread; // 필요 시 추가
    extern std::vector<std::atomic<long long>> total_inter_insert_ns_per_thread; // 필요 시 추가

    extern double get_avg_from_per_thread_ns(const std::vector<std::atomic<long long>>& arr);
}
...
#pragma omp parallel for num_threads(thread_count) schedule(dynamic)
  ...
  //  std::cout << "Average time for robin_set::clear(): "
	//                          << diskann::get_avg_from_per_thread_ns(diskann::total_rs_clear_ns_per_thread) << "s." << std::endl; // 헬퍼 함수 사용
    std::cout << "Average time for visited list::reset():      " // 공백 맞춤
		                          << diskann::get_avg_from_per_thread_ns(diskann::total_vl_reset_ns_per_thread) << "s." << std::endl; // 헬퍼 함수 사용
                             ...
                             const std::string &universal_label)
{
    ...
    diskann::reset_latency_counters();
...
int main(int argc, char **argv)
{
    ...
    std::cout << "\n--- Final Average Latencies (Incremental/Delete Phases) ---" << std::endl;
 //   std::cout << "Avg. time for robin_set::clear() (post-build): "
//	              << diskann::get_avg_from_per_thread_ns(diskann::total_rs_clear_ns_per_thread) << "s." << std::endl;
    std::cout << "Avg. time for visited list::reset()      (post-build): "
	              << diskann::get_avg_from_per_thread_ns(diskann::total_vl_reset_ns_per_thread) << "s." << std::endl;
    std::cout << "Avg. time for search_prune     (post-build): "
	              << diskann::get_avg_from_per_thread_ns(diskann::total_search_prune_ns_per_thread) << "s." << std::endl;
    std::cout << "Avg. time for inter_insert     (post-build): "
	              << diskann::get_avg_from_per_thread_ns(diskann::total_inter_insert_ns_per_thread) << "s." << std::endl;
    std::cout << "----------------------------------------------------------" << std::endl;
