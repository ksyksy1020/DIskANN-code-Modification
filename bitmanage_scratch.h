...
private:
    ...
    std::vector<uint32_t> _touched_buffer;
    uint32_t _cursor;
    size_t _num_bits;

public:
    SegmentedBitset(size_t size = 0) : _num_bits(0), _cursor(0) {
        if (size > 0) resize(size);
    }

    void resize(size_t num_bits) {
        _num_bits = num_bits;
        size_t num_words = (num_bits + 63) >> 6;
        _data.resize(num_words, 0);
        _touched_buffer.resize(num_words);
        _cursor = 0;
    }
    ...
    inline bool test(uint32_t id) const {
        return _data[id >> 6] & (1ULL << (id & 63));
    }

    inline void set(uint32_t id) {
        size_t word_idx = id >> 6;
        _data[word_idx] |= (1ULL << (id & 63));
        _touched_buffer[_cursor++] = word_idx;
    }

    void reset() {
        for (uint32_t i = 0; i < _cursor; ++i) {
            _data[_touched_buffer[i]] = 0;
        }
        _cursor = 0;
    }
};
