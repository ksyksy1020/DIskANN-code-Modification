...
#define MAX_POINTS_FOR_USING_BITSET 110000000
...
static double get_avg_from_per_thread_ns(const std::vector<std::atomic<long long>>& arr) {
...
auto is_not_visited = [this, fast_iterate, &inserted_into_pool_bs, &inserted_into_pool_rs](const uint32_t id) {
        return fast_iterate ? inserted_into_pool_bs.test(id) == false // <--- .test() 로 변경
