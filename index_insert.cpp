...
#define MAX_POINTS_FOR_USING_BITSET 1000000
...
//
//
std::vector<std::atomic<long long>> total_search_prune_ns_per_thread(MAX_THREADS);
std::vector<std::atomic<long long>> total_inter_insert_ns_per_thread(MAX_THREADS);

//extern std::vector<std::atomic<long long>> total_rs_clear_ns_per_thread;
//extern std::vector<std::atomic<long long>> total_bs_reset_ns_per_thread;

extern std::vector<std::atomic<long long>> total_vl_reset_ns_per_thread;

double get_avg_from_per_thread_ns(const std::vector<std::atomic<long long>>& arr) {
    long long total_ns = 0;
    int active_threads = 0;
    for (size_t i = 0; i < arr.size(); ++i) {
        long long val = arr[i].load();
        if (val > 0) {
            total_ns += val;
            active_threads++;
        }
    }
    if (active_threads == 0) return 0.0;
    return (double)total_ns / (double)active_threads / 1000000000.0;
}


void reset_latency_counters() {
        diskann::cout << "Resetting latency counters after initial build." << std::endl;
        for (size_t i = 0; i < MAX_THREADS; ++i) {
            total_search_prune_ns_per_thread[i].store(0);
            total_inter_insert_ns_per_thread[i].store(0);
 //           total_rs_clear_ns_per_thread[i].store(0);
 //           total_bs_reset_ns_per_thread[i].store(0);
            total_vl_reset_ns_per_thread[i].store(0);
        }
}
                                                      ...
                                                      uint32_t r, uint32_t maxc, size_t dim)
{
    ...
    {
       auto scratch = new InMemQueryScratch<T>(search_l, indexing_l, r, maxc, dim, _data_store->get_aligned_dim(), _data_store->get_alignment_factor(), total_internal_points, _pq_dist);
    ...
    const std::vector<LabelT> &filter_labels, bool search_invocation)
{
   ...
   // std::vector<uint8_t> &visited_nodes_array = scratch->visited_nodes_array();
  //  boost::dynamic_bitset<> &inserted_into_pool_bs = scratch->inserted_into_pool_bs();
    auto& visited_list = *(scratch->visited_list());
...
//    bool fast_iterate = true; // ++ for using bitset
...
//    }
    ...
    auto is_not_visited = [&visited_list](const uint32_t id) {
	    return visited_list.mass[id] != visited_list.curV;
    };
    ...
    for (auto id : init_ids)
    {
        ...
        if (is_not_visited(id))
        {
           ...
           // }
            visited_list.mass[id] = visited_list.curV;
...
#pragma omp parallel for schedule(dynamic, 2048)
	...
	//
        ...
        if (tid < MAX_THREADS) {
            total_inter_insert_ns_per_thread[tid] += std::chrono::duration_cast<std::chrono::nanoseconds>(insert_end - insert_start).count();
        }
	//
...
#pragma omp parallel for schedule(dynamic, 2048)
    ...
    if (_nd > 0)
    {
        ...
        diskann::cout << "--- Average Scratch Clear/Reset Times (per active thread) ---" << std::endl; // 제목 수정
       // diskann::cout << "Average time for robin_set::clear(): "
       //               << get_avg_from_per_thread_ns(total_rs_clear_ns_per_thread) << "s." << std::endl; // 헬퍼 함수 사용
       // diskann::cout << "Average time for bitset::reset():      " // 공백 맞춤
       //               << get_avg_from_per_thread_ns(total_bs_reset_ns_per_thread) << "s." << std::endl; // 헬퍼 함수 사용
        diskann::cout << "--- short array visited list reset  Times ---" << std::endl;
        diskann::cout << "Average time for VisitedList::reset(): " // "Total" -> "Average"
                      << get_avg_from_per_thread_ns(total_vl_reset_ns_per_thread) << "s." << std::endl;
        diskann::cout << "--- Average Link() Sub-task Timings (per active thread) ---" << std::endl; // 제목 수정
        ...
        diskann::cout << "Average time for inter_insert():                "
...
void Index<T, TagT, LabelT>::build_with_data_populated(const std::vector<TagT> &tags)
{
    ...
    if (_dynamic_index)
    {
	        diskann::cout << "Populating empty slots for dynamic index..." << std::endl;
		    _empty_slots.clear();
		        _empty_slots.reserve(_max_points);
			    for (auto i = _nd; i < _max_points; i++)
				        {
				    ...
				    diskann::cout << "Added " << _empty_slots.size() << " locations to empty slots." << std::endl;
    }
...
//	diskann::cout << "DEBUG (reserve_location): _empty_slots.size()=" << _empty_slots.size()
//		                      << ", _nd=" << _nd << ", _max_points=" << _max_points << std::endl;
