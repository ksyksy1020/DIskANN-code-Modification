...
std::vector<std::atomic<long long>> total_rs_clear_ns_per_thread(MAX_THREADS);
std::vector<std::atomic<long long>> total_bs_reset_ns_per_thread(MAX_THREADS);
//
//
template <typename T>
    ...
    : _L(0), _R(r), _maxc(maxc)
{
    ...
    _inserted_into_pool_bs = new SegmentedBitset<1024>(); 
...
template <typename T> void InMemQueryScratch<T>::clear()
{
    ...
    if (tid < MAX_THREADS) {
        total_rs_clear_ns_per_thread[tid] += std::chrono::duration_cast<std::chrono::nanoseconds>(rs_end - rs_start).count();
    }
    ...
    if (tid < MAX_THREADS) {
        total_bs_reset_ns_per_thread[tid] += std::chrono::duration_cast<std::chrono::nanoseconds>(bs_end - bs_start).count();
    }
