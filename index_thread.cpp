...
//std::atomic<long long> total_rs_clear_ns(0);
//std::atomic<long long> total_bs_reset_ns(0);
//std::atomic<long long> total_search_prune_ns(0);
//std::atomic<long long> total_inter_insert_ns(0);
                                                      ...
                                                      uint32_t r, uint32_t maxc, size_t dim)
{
    ...
    for (uint32_t i = 0; i < num_threads; i++)
    {
        auto scratch = new InMemQueryScratch<T>(search_l, indexing_l, r, maxc, dim, _data_store->get_aligned_dim(),
                                                _data_store->get_alignment_factor(), total_internal_points, _pq_dist);
...
#pragma omp parallel for schedule(dynamic, 2048)
	...
	auto insert_end = std::chrono::high_resolution_clock::now();
        total_inter_insert_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(insert_end - insert_start).count();
	//
...
//        diskann::cout << "Total time for robin_set::clear(): "
//                      << (double)total_rs_clear_ns.load() / 1000000000.0 << "s." << std::endl;
