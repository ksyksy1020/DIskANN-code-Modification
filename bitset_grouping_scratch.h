...
template<size_t SegSize = 1024>
class SegmentedBitset {
private:
    static constexpr size_t WORDS_PER_SEGMENT = SegSize / 64;
    static constexpr size_t BYTES_PER_SEGMENT = WORDS_PER_SEGMENT * sizeof(uint64_t);
...
public:
    SegmentedBitset(size_t size = 0) : _num_bits(0), _num_segments(0) { 
        if(size > 0) resize(size); 
    }
    void resize(size_t num_bits) {
        _num_bits = num_bits;
        _num_segments = (num_bits + SegSize - 1) / SegSize;
        _data.resize(_num_segments * WORDS_PER_SEGMENT, 0);
        size_t meta_words = (_num_segments + 63) / 64;
        _meta.resize(meta_words, 0);
    }

    size_t size() const { return _num_bits; }

    inline bool test(uint32_t id) const {
        size_t seg_idx = id / SegSize;
        size_t offset = id % SegSize;
        size_t word_idx = (seg_idx * WORDS_PER_SEGMENT) + (offset / 64);
        return _data[word_idx] & (1ULL << (offset % 64));
    }
    ...
    inline void set(uint32_t id) {
        ...
        if (_data[word_idx] & mask) return;
        _data[word_idx] |= mask;
        size_t meta_idx = seg_idx / 64;
        _meta[meta_idx] |= (1ULL << (seg_idx % 64));
    }

    void reset() {
        for (size_t i = 0; i < _meta.size(); ++i) {
            uint64_t word = _meta[i];
            if (word == 0) continue;
            for (int bit = 0; bit < 64; ++bit) {
                if (word & (1ULL << bit)) {
                    size_t seg_idx = i * 64 + bit;
                    if (seg_idx < _num_segments) {
                        std::memset(&_data[seg_idx * WORDS_PER_SEGMENT], 0, BYTES_PER_SEGMENT);
                    }
                }
            }
            _meta[i] = 0;
...
template <typename T> class InMemQueryScratch : public AbstractScratch<T>
{
  public:
    ...
    inline SegmentedBitset<1024>&inserted_into_pool_bs()
    {
  ...
  private:
    ...
    // of DiskANN.
    SegmentedBitset<1024> *_inserted_into_pool_bs;
