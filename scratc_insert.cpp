...
//
//static std::atomic<long long> total_bs_reset_ns;
//static std::atomic<long long> total_rs_clear_ns;
//
//std::atomic<long long> total_vl_reset_ns(0);
std::vector<std::atomic<long long>> total_vl_reset_ns_per_thread(MAX_THREADS);

template <typename T>
InMemQueryScratch<T>::InMemQueryScratch(uint32_t search_l, uint32_t indexing_l, uint32_t r, uint32_t maxc, size_t dim,
                                        size_t aligned_dim, size_t alignment_factor,size_t total_points, bool init_pq_scratch)
    : _L(0), _R(r), _maxc(maxc)
{
    ...
    _visited_list = new hnswlib::VisitedList(total_points + 1);
...
//    auto rs_start = std::chrono::high_resolution_clock::now();
//    _inserted_into_pool_rs.clear();
//  auto rs_end = std::chrono::high_resolution_clock::now();
//    total_rs_clear_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(rs_end - rs_start).count();
...
//    total_bs_reset_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(bs_end - bs_start).count();
    ...
    if (tid < MAX_THREADS)
    {
        total_vl_reset_ns_per_thread[tid] += std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count();
    }
    _id_scratch.clear();
    _dist_scratch.clear();
