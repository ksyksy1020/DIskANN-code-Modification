...
public:
    ...
    void resize(size_t num_bits) {
        ...
        _data.resize(_num_segments * SegSize);
        _data.reset();

        _meta.resize(_num_segments);
        _meta.reset();
    }
    ...
    inline void set(uint32_t id) {
        if (_data.test(id)) return;

        _data.set(id);
        _meta.set(id / SegSize);
    }

    void reset() {
        size_t seg_idx = _meta.find_first();

        while (seg_idx != _meta.npos) {
            _data.reset(seg_idx * SegSize, SegSize);
            seg_idx = _meta.find_next(seg_idx);
        }
...
template <typename T> class InMemQueryScratch : public AbstractScratch<T>
{
  public:
    ...
    inline SegmentedBitset<> &inserted_into_pool_bs()
    {
  ...
  private:
    ...
    // of DiskANN.
    SegmentedBitset<> *_inserted_into_pool_bs;
